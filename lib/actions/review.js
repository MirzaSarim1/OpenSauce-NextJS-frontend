    ...
    try {
        ...
        if (recipe && recipe.authorId !== session.user.id) {
            ...
            const message = comment
                ? `${reviewer.name} left a ${rating}-star review on your recipe "${recipe.title}": "${comment.substring(0, 100)}${comment.length > 100 ? '...' : ''}"`
                : `${reviewer.name} left a ${rating}-star review on your recipe "${recipe.title}"`
